...
class Strategy:
    ...
    def __init__(self, game, initial_strategy=None):
        ...
        self.numpy_file_str = 'S' + '_' + self.game.name + '.npy'
        # np.savez('S' + '_' + self.game.name, self.initial_strategy, self.cumulative_regret, self.cumulative_strategy)
        # np.save(self.numpy_file_str, np.zeros((len(MY_SAVING_RANGES), 3, 2, 3, 9)))
...
if __name__ == '__main__':
    ...
    #
    SK = Strategy(KK)
    SK.run_base_cfr(100000)
    SK0 = Strategy(KK)
    SK.run_base_cfr(10000)
    #    GK = Strategy(G)
