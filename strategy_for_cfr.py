...
class StrategyForCfr:
    ...
    def __init__(self, game, initial_strategy=None, naming_key=''):
        self.game = game
        ...
        self.strategy_data_array = np.zeros((n_of_saves, 2, 2, self.number_of_hands, self.number_of_nodes))
        self.strategy_data_path = os.path.join(ARRAYS_PATH, self.name + '.npy')
        self.save_points_file_path = os.path.join(ARRAYS_PATH, self.name + '_saves_points' + '.npy')
        # initialize array that saves cum_reg and cum_strat
        if not os.path.exists(self.strategy_data_path):
            np.save(self.strategy_data_path, self.strategy_data_array)

        # initialize array that saves save_points and save_time
        if not os.path.exists(self.save_points_file_path):
            arr = np.zeros((n_of_saves, 4))
            np.save(self.save_points_file_path, arr)

        #np.savez(self.name,
        #         initial_strategy=self.initial_strategy,
        #         cumuluative_regret=self.cumulative_regret,
...
# ------------------------------- SAVING AND READING INTO NUMPY ARRAYS NPZ FILE -------------------------------------- #
    def sync_from_strategy_data(self, save_point=None):
        arr_points = np.load(self.save_points_file_path)
        arr_data = np.load(self.strategy_data_path)
...
if __name__ == '__main__':
    ...
    sb = np.load(KUHN_ORIG_STRATEGY.strategy_data_path)
    ts = StrategyForCfr(KUHN_ORIG, np.zeros((2, 3, 9)))
    tts = StrategyForCfr(KUHN_ORIG, np.zeros((2, 3, 9)))
